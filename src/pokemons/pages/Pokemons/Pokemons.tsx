...
export const Pokemons = () => {
  ...
  const setPage: (page: number) => void = usePokemonsState(state => state.setPage);

  const { data: { results: pokemons, count }} = useSuspenseQuery({
    queryFn: () => getPokemons ({ itemsPerPage: defaultItemsPerPage, offset: computePaginationOffset(page, defaultItemsPerPage)}),
